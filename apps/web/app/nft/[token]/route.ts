...
// Serves ERC-721 metadata for the GomokuDawgs membership pass, matching the
...
export function GET(request: Request, { params }: { params: { token: string } }) {
  ...
  const metadata = {
    name: Number.isFinite(tokenId) ? `Gomoku Dawgs Pass #${tokenId}` : "Gomoku Dawgs Pass",
    description:
      "Membership pass for GomokuDawgs — wagered five-in-a-row in the Deputy Dawgs ecosystem. Holding a pass is your seat at the board.",
    image: `${origin}/assets/nft-pass.svg`,
    external_url: origin,
    ...(Number.isFinite(tokenId) ? { tokenId } : {}),
    attributes: [
      { trait_type: "Collection", value: "Gomoku Dawgs" },
